...
dataset = pd.read_csv('resources/phone_cost.csv',
                      names=['blue', 'clock_speed', 'dual_sim', 'fc', 'four_g', 'm_dep', 'n_cores', 'pc',
                             'sc_h', 'sc_w', 'talk_time',
                             'three_g', 'touch_screen', 'wifi',
                             'price_range', 'battery_power', 'px_height', 'px_width', 'ram', 'mobile_wt', 'int_memory', ])  # Import all columns omitting the fist which consists the names of the animals


# We drop the animal names since this is not a good feature to split the data on
#dataset = dataset.drop('battery_power', axis=1) #So I just added these values to the end of the dataset and my prediction went from 1percent to 100percent...
#dataset = dataset.drop('px_height', axis=1)
#dataset = dataset.drop('px_width', axis=1)
#dataset = dataset.drop('ram', axis=1)
#dataset = dataset.drop('mobile_wt', axis=1)
#dataset = dataset.drop('int_memory', axis=1)
...
def InfoGain(data, split_attribute_name, target_name="price_range"):
    """
...
def ID3(data, originaldata, features, target_attribute_name="price_range", parent_node_class=None):
    """
...
def train_test_split(dataset):
    training_data = dataset.iloc[:1800].reset_index(drop=True)  # We drop the index respectively relabel the index
    # starting form 0, because we do not want to run into errors regarding the row labels / indexes
    testing_data = dataset.iloc[1800:].reset_index(drop=True)
...
def test(data, tree):
    ...
    print('The prediction accuracy is: ', (np.sum(predicted["predicted"] == data["price_range"]) / len(data)) * 100,
          '%')
